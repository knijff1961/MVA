    ...
    case 0: // State: Led is aan
      switch(cPreEmptiveTimer->iSubState) {
        case STATE_INIT:
          ...
          break;
        case STATE_WAIT:
          if(cPreEmptiveTimer->timeout(currentMillis)) { // Is event opgetreden?
            // Zet de nieuwe state
            // setNewState() zet automatisch iSubState = STATE_WAIT
            cPreEmptiveTimer->setNewState(1);
          }
          break;
      }
    case 1: // State: Led is uit
      switch(cPreEmptiveTimer->iSubState) {
        case STATE_INIT:
          digitalWrite(iPinLed, LOW);                   // LED aan
